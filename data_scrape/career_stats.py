...
class CareerStatsScraper(AbstractBaseScraper):
    COLUMN_TYPES: dict[str: str] = {
        "Season": "str", "Tm": "str", "PTS": "float", 
        "AST": "float", "TRB": "float", "FT": "float", "THPA": "float", 
        "THP": "float", "FGA": "float", "FG": "float", "STL": "float",
        "FG_perc": "float", "THP_perc": "float", "TWP": "float", 
        "TWPA": "float", "TWP_perc": "float", "eFG_perc": "float",
        "FTA": "float", "FT_perc": "float", "ORB": "float", "DRB": "float",
        "AST": "float", "STL": "float", "BLK": "float", "TOV": "float", 
        "PF": "float", "Awards": "str", "Age": "float", "Lg": "str", 
        "Pos": "str", "G": "int", "GS": "int", "MP": "float"
    }
    
    STAT_AUGMENTATIONS: dict[str: str] = {
        "PA": "PTS+AST", "PR":"PTS+TRB", "RA":"TRB+AST", "PRA":"PTS+TRB+AST",
        "Season": lambda dataset: convert_seasons_to_years(dataset=dataset, id_from_column="Season"),
    }
    ...
    RENAME_COLUMNS = {
        "FG%": "FG_perc", "3P": "THP", "3PA": "THPA", "3P%": "THP_perc",
        "2P": "TWP", "2PA": "TWPA", "2P%": "TWP_perc", "eFG%": "eFG_perc",
        "FT%": "FT_perc"
    }
    ...
    def select_dataset_from_html_tables(self, *, datasets: list[pd.DataFrame]) -> pd.DataFrame:
        ...
        season_stat_datasets: list[pd.DataFrame] = list(filter(lambda dataset: "Season" in dataset.columns, datasets))
    ...
    def get_active_seasons(self) -> list[int]:
        try:
            ...
            season_stats: pd.DataFrame = self.data[self.data["Season"].str.contains('^\d{4}') == True]
