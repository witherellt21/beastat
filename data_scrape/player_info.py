...
def get_player_ids(*, source_table: pd.DataFrame, id_from_column: str) -> pd.Series:    
    ...
    for id in duplicate_ids:
        id_rows: pd.DataFrame = source_table[source_table["player_id"] == id].sort_values("active_from")
...
class PlayerInfoScraper(AbstractBaseScraper):
    ...
    STAT_AUGMENTATIONS = {
        "player_id": lambda dataset: get_player_ids(source_table=dataset, id_from_column="name"),
        "name": lambda dataset: decode_player_name(source_table=dataset, id_from_column="name"),
        "height": lambda dataset: convert_height_to_inches(source_table=dataset, id_from_column="height"),
