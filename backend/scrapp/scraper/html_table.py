...
class HTMLTableArgs(TypedDict):
    ...
    multi_index_mapper: NotRequired[Callable[[tuple[str, str]], str]]
...
class BaseHTMLTable(
    ...
    Arguments:
        name (`str`): A name for the table. Used in dependency tree graphing.
        db_table (`BaseTable`): The database table that the data should be saved in.
        serializer (`BaseHTMLTableSerializer`): The serializer that should be used to validate raw data for the database.
        identification_function (`f(list[pd.DataFrame]) -> pd.DataFrame || None`): A function that extracts the singular table from a given set of tables (returned from a webpage).
        *nan_values (`list[str]`): A list of values that should be considered null in the table.
        *cached_query_generator (`f(QueryArgs || None) -> pd.DataFrame`): A function that generates a table from memory if the query has already occured.
    """
    ...
    def __init__(
        ...
        **kwargs: Unpack[HTMLTableArgs],
        ...
        self.multi_index_mapper = kwargs.get(
            "multi_index_mapper", self.__class__.MULTI_INDEX_MAPPER
        )
    ...
    def clean(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        **Override this for anything you want to be done to the dataset BEFORE saving.
        ...
        if isinstance(data.columns, pd.MultiIndex):
            # TODO: NEED TO FIX THE QUERY ARGS USAGE FOR MULTI-INDEX Tables
            data.columns = data.columns.map(self.multi_index_mapper).str.strip("_")
        ...
        # if self.name == "NFLPlayerRushingAndReceivingSplits":
        #     # str_io = StringIO()
        #     # data.info(buf=str_io)
        #     # raise Exception(str_io.getvalue())
        #     raise Exception(data.columns)

        try:
            data = self.serializer.execute(data)
    ...
    def push(self) -> None:
        ...
        # if self.name == "NFLKickAndPuntReturnsSplits":
        #     str_io = StringIO()
        #     data.info(buf=str_io)
        #     raise Exception(str_io.getvalue())
